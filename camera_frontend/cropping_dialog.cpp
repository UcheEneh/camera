    ...
    ui(new Ui::cropping_Dialog)
{
    ...
    connect(ui->pushButton_Save, &QPushButton::clicked, this, &cropping_Dialog::on_Edit);
...
void cropping_Dialog::onClippedImage(const QPixmap& pixmap)
{
    ...
    /*
    // TRY PUT CROP ON CLIPSCENE QGRAAPHICSVIEW
    ClipScene *m_clipScene = new ClipScene(this);
    m_clipScene->setImage2(pixmap.scaled(w,h,Qt::KeepAspectRatio,
                                         Qt::SmoothTransformation));
    ui->m_graphicsView->setScene(m_clipScene);  //set Graphics View as a clipping region
    */
...
void cropping_Dialog::on_Edit()
{
    if (ui->label_Image->isEnabled()){
        const QPixmap *pic(ui->label_Image->pixmap());

        Edit_Dialog edit;
        edit.setData(pic, global_fname); //, new_file);
        edit.setModal(true);
        edit.setWindowTitle("Edit Info");
        edit.exec();
    }
